...
Game::Game(sf::RenderWindow *window) : mt{rd()}{
    ...
    // CARGAR TIPO DE LETRA
    cargar(this->font,T_FONT);
    // CARGAR IMAGENES DE CADA OBJETO
...
void Game::initUI(){
    sf::Text follPlaText, scoreText, continueText, pauseText;
    // INIT FOLLOW PLAYER TEXTS
    for (int i = 0; i < this->players.size(); ++i) {
        confText(follPlaText, &this->font, 14, &this->followPlayerTexts);
    }
    // INIT STATIC TEXT
    confText(scoreText, &this->font, 14, &this->staticPlayerTexts);
    confText(continueText, &this->font, 14, &this->staticPlayerTexts,
            "Pulsa C para continuar",sf::Color::Yellow, {800.0f,0.0f});
    confText(pauseText, &this->font, 14, &this->staticPlayerTexts,
             "Pulsa P para pausar",sf::Color::White, {1400.0f,0.0f});
    // GAME OVER TEXT
...
void Game::updateUI() {
    ...
    staticPlayerTexts[0].setString("Score: " + std::to_string(this->score)); // SCORE
}
void Game::update() {
    // PARA SABER SI ESTAMOS EN EL MENU
    if (wind.get_WINDOW() == menu){
        wind.update(this->exit);
    }
    // PARA SABER SI ESTAMOS EN EL JUEGO
    if(wind.get_WINDOW() == game){
        if((this->players[0].getHP() > 0) && (this->score < GameConfig::WIN_SCORE)){
            enemyupdate();
            playerupdate();
        } else {
            // Se deja de actualizar el jugador y los enemigos y la musica se pausa
            this->music.pause();
        }
    }
}
...
void Game::enemyupdate(){
    ...
    if(GameConfig::CURRENT_ENEMY_TIME == GameConfig::ENEMY_TIME_MAX){
        this->enemies.emplace_back( Enemy(&enemyTexture,
                ...
                enemy1 , EnemyConfig::ENEMY_LIFE) );
        GameConfig::CURRENT_ENEMY_TIME = 0; // Reset enemy timer appear
    }
    // // BUCLE FOR PARA QUE SE CUMPLA PARA CADA ENEMIGO
...
void Game::colissionBulletEnemy(){
    ...
    for (int i = 0; i < players.size(); ++i) {
        ...
        for (int k = 0; k < this->players[i].getBullets().size(); ++k) {
            ...
            for_each (all(enemies),[&](Enemy & enemie) {
                if(this->players[i].getBullets()[k].getGlobalBounds().intersects(enemie.getGlobalBounds()) ){
                    enemie.takeDamage(EnemyConfig::ENEMY_DANO); // Enemigo recibe dano al chocar con la bala
...
// Las funciones anteriores ya tienen su bucle for para cada jugador dentro de la funcion por eso no entraran en el bucle for que se presenta a continuacion
    // BUCLE FOR PARA QUE SE CUMPLA PARA CADA JUGADOR
    for_each (all(players),[&](Player & player) {
...
void Game::draw() {
    ...
    if(wind.get_WINDOW() == game){
        // DIBUJAR JUGADOR(ES)
        for (auto & player : this->players) { player.draw(*this->window); }
        // DIBUJAR ENEMIGOS
        for(auto & enemie : this->enemies){ enemie.draw(*this->window); }
